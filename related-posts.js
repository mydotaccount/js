...
(async function() {
  ...
  const feedUrl = `${window.location.origin}/feeds/posts/default/-/${labelSlug}?alt=json&max-results=30`;
  ...
  try {
    ...
    for (let e of entries) {
      ...
      // استخراج برچسب‌ها از هر پست
      const labels = e.category ? e.category.map(cat => cat.term.trim()) : [];

      // تطابق دقیق: حداقل یکی از برچسب‌ها دقیقا با یکی از برچسب‌های فعلی برابر باشد
    ...
    // نمایش حداکثر ۴ پست به‌صورت تصادفی
